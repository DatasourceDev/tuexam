...
@Component({
  selector: 'app-question-read-text-multi-choice',
  templateUrl: './question-read-text-multi-choice.component.html',
  styleUrls: ['./question-read-text-multi-choice.component.css']
})
export class QuestionReadTextMultiChoiceComponent implements OnInit {
  ...
  OnQuestionAdd() {
    if (this.id == null || parseInt(this.id) == 0) {
      ...
      this.router.navigate(['/question-question/', this.id]);
      return false;
    }
  }
  OnQuestionEdit(id, questiontype) {
    if (questiontype == '1')
      this.router.navigate(["/question-multi-choice/", id, this.id]);
    else if (questiontype == '2')
      this.router.navigate(["/question-tf/", id, this.id]);
    else if (questiontype == '3')
      this.router.navigate(["/question-multi-math/", id, this.id]);
    else if (questiontype == '4')
      this.router.navigate(["/question-short-ans/", id, this.id]);
    else if (questiontype == '5')
      this.router.navigate(["/question-eassy/", id, this.id]);
    else if (questiontype == '6')
      this.router.navigate(["/question-assigment/", id, this.id]);
    else if (questiontype == '7')
      this.router.navigate(["/question-read-text-multi-choice/", id, this.id]);
    else if (questiontype == '8')
      this.router.navigate(["/question-attitude/", id, this.id]);
 ...
 */
        ...
        this.service.httpClientPost("api/Question/insert", formdata)
          .subscribe(result => {
            if (result == null || Object.keys(result).length == 0 || (Array.isArray(result) && result.length == 0)) {
              ...
              if (result["result"] == 200) {
                if (state == 1) {
                  this.router.navigate(['/question-question/', result["id"]]);
                }  
                else {
                  this.router.navigate(['/question-search/']);
                }
  ...
  OnFileDelete() {
    Swal.fire({ text: 'คุณต้องการที่จะไฟล์นี้', type: 'warning', showCancelButton: true, cancelButtonText: 'ยกเลิก', confirmButtonText: 'ตกลง', buttonsStyling: false, customClass: { confirmButton: 'btn btn-blue', cancelButton: 'btn btn-white' } }).then((result) => {
      if (result.value == true) {
        let deldata = { id: this.id, update_by: this.useraccesdata.username};

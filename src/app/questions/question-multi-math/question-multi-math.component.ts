...
@Component({
  selector: 'app-question-multi-math',
  templateUrl: './question-multi-math.component.html',
  styleUrls: ['./question-multi-math.component.css']
})

export class QuestionMultiMathComponent implements OnInit {
  ...
  OnQuestionAdd() {
    if (this.id == null || parseInt(this.id) == 0) {
      ...
      this.router.navigate(['/mm-sub-question/', 0, this.id]);
      return false;
    }
  }
  OnQuestionEdit(id, questiontype) {
    this.router.navigate(["/mm-sub-question/", id, this.id]);
    return false;
  }
  OnQuestionDelete(id) {
  ...
  OnQuestionMoveUp(id) {
    let formdata = { id: id };
    this.service.httpClientGet("api/Question/moveup", formdata)
      .subscribe(result => {
        this.OnQuestionSearch();
      }, error => {
        Swal.fire({ text: 'เกิดข้อผิดพลาดในระบบ', type: 'error', confirmButtonText: 'ตกลง', buttonsStyling: false, customClass: { confirmButton: 'btn btn-danger' } });
  ...
  OnQuestionmMoveDown(id) {
    let formdata = { id: id };
    this.service.httpClientGet("api/Question/movedown", formdata)
      .subscribe(result => {
        this.OnQuestionSearch();
      }, error => {
        Swal.fire({ text: 'เกิดข้อผิดพลาดในระบบ', type: 'error', confirmButtonText: 'ตกลง', buttonsStyling: false, customClass: { confirmButton: 'btn btn-danger' } });
  ...
  OnAnsAdd() {
    if (this.id == null || parseInt(this.id) == 0) {
      ...
      this.router.navigate(['/mm-sub-answer/',0, this.id]);
      return false;
    }
  }
  OnAnsEdit(id) {
    this.router.navigate(["/mm-sub-answer/", id, this.id]);
    return false;
  }
  OnAnsDelete(id) {
    Swal.fire({ text: 'คุณต้องการที่จะลบรายการนี้', type: 'warning', showCancelButton: true, cancelButtonText: 'ยกเลิก', confirmButtonText: 'ตกลง', buttonsStyling: false, customClass: { confirmButton: 'btn btn-blue', cancelButton: 'btn btn-white' } }).then((result) => {
        ...
        this.service.httpClientPost("api/Question/insert", formdata)
          .subscribe(result => {
            if (result == null || Object.keys(result).length == 0 || (Array.isArray(result) && result.length == 0)) {
              ...
              if (result["result"] == 200) {
                if (state == 1) {
                  this.router.navigate(['/mm-sub-question/',0, result["id"]]);
                }
                else {
                  this.router.navigate(['/question-search/']);
                }

...
declare var $: any;
@Component({
  selector: 'app-reset-password-student',
  templateUrl: './reset-password-student.component.html',
  styleUrls: ['./reset-password-student.component.css']
})
export class ResetPasswordStudentComponent implements OnInit {
  ...
  OnSubmit() {
    ...
    if (this.inputForm.valid) {
      ...
      this.loading = true;
      if (this.id != null && parseInt(this.id) > 0) {
        this.service.httpClientPost("api/Student/resetpassword", formdata)
          .subscribe(result => {
            if (result == null || Object.keys(result).length == 0 || (Array.isArray(result) && result.length == 0)) {

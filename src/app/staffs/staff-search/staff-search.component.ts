...
export class StaffSearchComponent implements OnInit {
  ...
  constructor(private service: AppService, private http: HttpClient, private appdata: AppData, private router: Router) {
    ...
    let role_search = new FormControl('');
  ...
  ngOnInit() {
    ...
    this.rolelist = this.appdata.getrole();
  }
  OnSearch(formdata) {
    this.loading = true;

    this.service.httpClientGet("api/Staff/liststaff", formdata)
      .subscribe(result => {
  ...
  OnSubmit() {
    ...
    this.SearchFrom.patchValue({ status_search: status_search });
    this.SearchFrom.patchValue({ role_search: role_search });

    let formdata = {
      text_search: this.SearchFrom.value.text_search,
      status_search: this.SearchFrom.value.status_search,
      role_search: this.SearchFrom.value.role_search,
    };
  ...
  OnDelete(id) {
    Swal.fire({ text: 'คุณต้องการที่จะลบรายการนี้', type: 'warning', showCancelButton: true, cancelButtonText: 'ยกเลิก', confirmButtonText: 'ตกลง', buttonsStyling: false, customClass: { confirmButton: 'btn btn-blue', cancelButton: 'btn btn-white' } }).then((result) => {
      if (result.value == true) {
        let deldata = { id: id };
        this.service.httpClientGet("api/Staff/delete", deldata)
          .subscribe(result => {

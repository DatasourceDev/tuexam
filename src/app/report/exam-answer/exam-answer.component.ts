...
@Component({
  ...
  styleUrls: ['./exam-answer.component.css']
})
export class ExamAnswerComponent implements OnInit {
  ...
  OnExcel(id) {
      this.service.openurl("api/ExcelReport/examanswer?examid=" + id);
    return false;
  }
  OnPdf(id) {
      this.service.openurl("api/PdfReport/examanswer?examid=" + id);
    return false;
  }
  OnPageChange(no) {
    if (no < 1)

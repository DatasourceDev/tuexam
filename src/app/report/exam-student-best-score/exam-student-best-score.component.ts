...
@Component({
  selector: 'app-exam-student-best-score',
  templateUrl: './exam-student-best-score.component.html',
  styleUrls: ['./exam-student-best-score.component.css']
})
export class ExamStudentBestScoreComponent implements OnInit {
  ...
  OnSearch(formdata) {
    ...
    this.service.httpClientGet("api/Report/examstudentformbest", formdata)
      .subscribe(result => {
  ...
  OnExcel() {
    this.service.openurl("api/ExcelReport/examstudentformbest?student_search=" + this.id);
    return false;
  }
  OnPdf() {
    this.service.openurl("api/PdfReport/examstudentformbest?student_search=" + this.id);
    return false;
  }
  OnPageChange(no) {
    if (no < 1)

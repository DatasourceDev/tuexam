...
@Component({
  selector: 'app-subjectsub-search',
  templateUrl: './subjectsub-search.component.html',
  styleUrls: ['./subjectsub-search.component.css']
})
export class SubjectsubSearchComponent implements OnInit {
  ...
  OnSearch(formdata) {
    ...
    this.service.httpClientGet("api/SubjectSub/listsub", formdata)
      .subscribe(result => {
        if (result == null || Object.keys(result).length == 0 || (Array.isArray(result) && result.length == 0)) {
          this.data = null;
          this.pagelen = 0;
  ...
  OnEdit(id) {
    this.router.navigate(['/subjectsub/', id]);
  }
  OnDelete(id) {
    Swal.fire({ text: 'คุณต้องการที่จะลบรายการนี้', type: 'warning', showCancelButton: true, cancelButtonText: 'ยกเลิก', confirmButtonText: 'ตกลง', buttonsStyling: false, customClass: { confirmButton: 'btn btn-blue', cancelButton: 'btn btn-white' } }).then((result) => {
      if (result.value == true) {
        ...
        this.service.httpClientGet("api/SubjectSub/delete", deldata)
          .subscribe(result => {
